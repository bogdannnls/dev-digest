...
| `security-reviewer` | `sonnet` | read-only | Adversarial OWASP-driven diff reviewer. Uses Anthropic's `/security-review` 10-category taxonomy; findings carry separate `severity` (Critical/High/Medium/Low) and `confidence` axes; every finding requires a traceable source → sink dataflow (no pattern-only flags). OWNS all OWASP-Top-10-taggable findings — `architecture-reviewer` excludes them. |
| `systematic-debugger` | `sonnet` | read-only (runs tests) | Hypothesis-driven root-cause diagnosis. Runs the four-phase loop (Investigation → Pattern Analysis → Hypothesis → Fix Proposal). Stops at "root cause + fix proposal drafted" and hands off a structured Debugging Report to the controller for `implementer` dispatch. Numeric stop rule at 3 unsuccessful attempts. Never applies fixes. |
...
- 2026-07-04 — Third batch on branch `l03`. Added `security-reviewer` and `systematic-debugger` agents, plus two companion skills — `sse-patterns` and `session-scope-guardian` — both under `.claude/skills/` (skills, not agents; not listed in the table above). Boundary decision: `security-reviewer` OWNS all OWASP-Top-10-taggable findings (injection, authZ, crypto, secrets, unsafe deserialization); `architecture-reviewer` explicitly excludes OWASP-taggable findings. This does NOT collapse `security-reviewer` and `api-contract-reviewer` — the former is OWASP/exploitability-driven, the latter is contract-schema-only. `systematic-debugger` is deliberately self-contained: it encodes the `obra/superpowers:systematic-debugging` four-phase discipline and numeric stop rule (2 → back to Phase 1, 3+ → recommend re-plan) directly in its own prompt, rather than depending on the global `superpowers:systematic-debugging` skill being loaded — reason: this repo may run in environments without superpowers installed. `sse-patterns` codifies the `done`-ordering race, the `runBus.complete()` method-name gotcha, the `RunEventKind` exhaustiveness coupling, and the `.inject()`-vs-real-socket testing trap, sourced from `server/INSIGHTS.md` (2026-06-24) and root `INSIGHTS.md` (2026-06-23); it is a rules-catalogue skill following the `onion-architecture` template. `session-scope-guardian` is a behavioral/workflow-discipline skill (not a rules catalogue) that detects task-scope drift — BLOCKING in subagent context (returns `blocked: task-scope-shift`), ADVISORY in main-session context (surfaces a scope-drift alert to the user, does not gate). It is an internal design pattern with no external cited source. Sources for the third batch: `anthropics/claude-code-security-review` GitHub Action + `/security-review` slash command (security-reviewer taxonomy + false-positive exclusion list); OWASP Top 10:2025 (`security-reviewer` diff-shaped signal checklist); Semgrep's severity/confidence rubric (severity-vs-confidence split); Snyk/Endor Labs dataflow-based anti-noise framing (require exploitable path); Anthropic's `/security-review` prompt-injection disclaimer (adversarial self-defense rule); Zeller, *Why Programs Fail* (2005/2009) and Zeller & Hildebrandt, *IEEE TSE* 2002 (`systematic-debugger` scientific-method loop + delta debugging); `obra/superpowers:systematic-debugging` (four-phase discipline + stop rule); arXiv 2506.18824 (anti-shotgun-debugging rule); Fastify docs + Liran Tal's blog on `reply.raw`/`reply.hijack()` (`sse-patterns` MUST.5); MDN Server-sent events + Anthropic streaming API docs (`sse-patterns` terminal-event ordering rule). Full source list embedded in each artifact.
