...
## 2026-08-25 — A literal NUL byte in `blast-graph.ts` makes git classify a TS source file as binary, so it never shows a diff
Context: reviewing the merge of L04's Blast Radius work into l05. `git diff --stat` reported `blast-graph.ts | Bin 0 -> 2608 bytes` for a plain TypeScript file.
What we tried: assumed a tooling glitch or a stray encoding, since the file typechecks and its tests pass.
What worked: `python3 -c "print(open(f,'rb').read().count(b'\x00'))"` → 1. There is a real NUL (0x00) control byte at byte offset 1506, line 33 of `client/src/app/repos/[repoId]/pulls/[number]/_components/OverviewTab/_components/BlastRadiusCard/blast-graph.ts`, inside a composite map key: `` const key = `${prefix}<NUL>${label}`; ``. The intent is sound — NUL is a good delimiter for a composite key because it cannot occur in an identifier — but it was committed as a raw control byte instead of the two-character escape `\0`.
Why it matters: git's binary heuristic keys on NUL, so this file renders as `Bin` in `git diff`, `git show`, and PR review — every future change to it is invisible to reviewers, on a file that holds real graph-building logic. `pnpm typecheck` and the 20 BlastRadiusCard tests all pass, so no gate catches it. If you use a control character as a delimiter, write the escape (`\0`, `\x1f`) rather than pasting the byte; and if a source file ever shows as `Bin` in a diffstat, check for NUL before assuming a tooling problem.
- 2026-08-26 · What Doesn't Work · The eval case modal's "recall/precision/citation" line renders three em-dashes for every case, always — it reads `lastRun.recall` etc., and the server never writes those per-run columns (only the batch-level ones). client/src/app/agents/[id]/_components/AgentEditor/_components/EvalsTab/_components/EvalCaseModal/EvalCaseModal.tsx:93. Reason: the null-coalescing to EM_DASH is correct code and the batch metrics elsewhere in the UI are populated, so this reads as "this case has no data yet" rather than as a field that can never be filled.
- 2026-08-27 · Codebase Patterns · A vendor `TextInput` cannot be made to shrink: `<input type="number">` has a ~168px min-content width in Chrome (spinner included), grid/flex items default to `min-width: auto`, and the usual escape — passing `style={{minWidth:0}}` — is silently dropped because the component spreads `{...rest}` BEFORE its own `style`. client/src/vendor/ui/kit/TextInput.tsx:36. Reason: the file is a do-not-touch vendor zone, so any narrow track holding one of these will push its container wider (79px in the eval case modal) and the fix has to be a wider track from the caller's side, not a constraint on the input.
- 2026-08-27 · What Doesn't Work · `EvalRunsTable` sets fixed px columns totalling ~834px with `overflow: hidden`, which fits the full-width Eval Dashboard but silently CLIPS Pass/Cost/Status in the agent editor's ~718px pane — the columns become unreachable rather than scrollable. client/src/components/eval/EvalRunsTable/styles.ts:12. Reason: the component is shared by a wide host and a narrow one, so it looks correct wherever you would naturally check it; `overflow: hidden` was there to clip the border radius and quietly doubled as data loss.
- 2026-08-27 · Codebase Patterns · `Sparkline` guards the empty series (`if (!data.length) return null`, Sparkline.tsx:15) but not the one-point series: x is spaced as `i / (data.length - 1)`, so a single run divides zero by zero and every coordinate becomes NaN ("Received NaN for the `cx` attribute"). client/src/vendor/ui/charts/Sparkline.tsx:19. Reason: the empty-case guard reads as "degenerate input is handled", which is exactly why callers do not think to check; the file is a do-not-touch vendor zone so every call site must require >= 2 points itself, and the sibling `LineChart` does NOT share the flaw (Recharts renders one point fine), so the two chart components have different contracts for degenerate input.
