...
| Name | Shape | Notes |
|---|---|---|
| `PrWhyRiskBrief` (new, `shared/contracts/brief-synth.ts`) | `{ what: string, why: string, riskLevel: RiskSeverity, risks: RiskArea[], reviewFocus: ReviewFocusItem[], model: string, cost: {tokensIn: number, tokensOut: number, usd: number}, computedAt: string, basedOn: {headSha: string, reviewId: string \| null, intentComputedAt: string} }` | Reuses the existing `RiskSeverity` enum from `contracts/brief.ts` — does not redefine it (AC-1). `reviewId` is nullable: the anchor review can be deleted (FK `ON DELETE SET NULL`) after compute, and a served brief may outlive it. |
