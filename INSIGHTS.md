...
- 2026-08-25 · What Doesn't Work · A prompt change that reads as a strict improvement can be a large regression: banning coverage-gap findings AND capping output at 3 findings dropped the Test Quality Reviewer's eval recall 67%→17% while precision moved only 2pp — the cap made the model emit one finding per case, usually not the expected one. Splitting the two changes (ban kept, cap dropped) recovered recall and beat baseline precision. docs/features/2026-08-25-eval-pipeline/demo-runbook.md:. Reason: the prompt diff argues the opposite of the measurement, so nothing but running the set surfaces it — and bundling two prompt edits into one run makes the eval unable to attribute the movement.
- 2026-08-26 · Tool & Library Notes · CI has never run in the `bogdannnls/dev-digest` fork: `actions/runs` reports `total_count: 0` for the whole repo lifetime, because GitHub disables Actions on forks until the owner enables them in the Actions tab. The misleading signal is that `actions/workflows` lists all five as `state: active` — that field is the per-workflow switch, not the repo-level one, so triggers and path filters look correct while nothing fires. .github/workflows/server-unit.yml:. Reason: a PR with zero checks reads as "path filters didn't match" and sends you auditing `on:` blocks that are fine; also a PAT without admin scope gets 403 on `actions/permissions`, so the flag itself cannot be read back to confirm.
- 2026-08-27 · What Doesn't Work · A type error inside a test file rides through a green commit: vitest transpiles without typechecking, and the PreToolUse commit gate runs only the vitest lanes, never `pnpm typecheck`. .claude/hooks/pre-commit-test-gate.sh:81. Reason: the test passes, the gate passes, and the break surfaces later in an unrelated session running typecheck for its own reasons — so "tests are green" is not evidence the branch compiles.
