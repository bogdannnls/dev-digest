...
## 2026-07-11 — `routeModel`'s Provider is `openai|anthropic` only; openrouter workspaces can't use its cheap-tier ids
Context: wiring the blast-radius one-paragraph LLM summary — the FIRST real consumer of `routeModel` (`server/src/platform/model-router.ts`, previously zero call sites).
What we tried: `resolveFeatureModel(ws, feature)` → `routeModel('summary', provider)` → `llm.complete`, assuming it works for any configured provider.
What worked: guard `if (provider === 'openrouter') return ''` BEFORE `routeModel`/`llm` in `OverviewService.generateBlastRadiusSummary` (`server/src/modules/overview/service.ts`). A fallback of `routeModel('summary','anthropic')` for openrouter is NOT enough — it sends a claude id to the openrouter client, which also fails.
Why it matters: `resolveFeatureModel`'s `Provider` union includes `'openrouter'`, but `routeModel`'s `Provider` is only `'openai' | 'anthropic'`, and its `'summary'`/`'intent'` tiers return bare ids like `claude-haiku-4-5` / `gpt-4o-mini` that are NOT valid OpenRouter slugs (`vendor/model`). So routing an openrouter workspace through `routeModel` is a guaranteed dead round-trip (+ a warn log) on every call. The mismatch is invisible until an openrouter-configured workspace hits the path. More feature summaries are likely to reuse `routeModel`; each must skip openrouter or `model-router.ts` must grow an openrouter table.
...
## 2026-07-11 — SUPERSEDES the above "GLOBAL cap" detail: the caller cap is now PER changed symbol
The entry above says the cap is GLOBAL (`slice(0, 20)`). Commit `85cea5c` changed it: callers are now capped per `(viaFile|viaSymbol)` key on both the persistent and ripgrep paths (`capCallersPerSymbol`, `repo-intel/service.ts`). The rest of that entry still holds — `impactedEndpoints`/`factsByFile` are still computed from the UNCAPPED caller-file set, so the "gate the fallback on `factsByFile` presence, not attribution count" lesson is unchanged.

## 2026-07-11 — repo-intel line numbers are relative to `lastIndexedSha`, NOT the PR head — forge deep links must pin to the index sha
Context: the Blast Radius card linked caller `file:line` to the PR's `head_sha`; users reported clicks opening the WRONG line of code.
What worked: surface the index sha end-to-end — `BlastResult.indexedSha` (set from `IndexState.lastIndexedSha` in `tryPersistentBlast`, `repo-intel/service.ts`), carried on the overview envelope, and the client pins caller links to `indexedSha ?? headSha` (`client/.../BlastRadiusCard.tsx`). Fixed in commit `df9b173`.
Why it matters: every position repo-intel emits (`references.line`, symbol line, caller line) is parsed by the indexer at the commit it indexed — `repo_index_state.lastIndexedSha`, i.e. the repo's poll-time HEAD, almost never equal to a given PR's `head_sha`. Caller files aren't even in the PR's changed set, so their content at the PR head differs arbitrarily → a deep link built with `head_sha` opens the wrong line. ANY feature that turns a repo-intel position into a forge blob link (blast today, code-nav tomorrow) MUST pin to the indexed sha, not the PR head. The bug hides whenever the index sha equals the PR head (e.g. the newest PR) — exactly why it slipped past earlier testing (demo PR #5's head WAS the indexed sha).

## 2026-07-12 — `pnpm typecheck` doesn't cover `test/**`, so test-double interface drift only bites at runtime
Context: L05 widened the `GitClient` port with `cloneExists`/`statFile`/`walkFiles`; several integration tests each carry their own `class RealDirGitClient implements GitClient` double (one copy per `*.it.test.ts` — `context-reader`, `run-executor-context`, `agents-context-paths`, `skills-context-paths`, `agents-versions`).
What we tried: relied on `pnpm typecheck` to flag any double missing the new methods after adding them to the port.
What worked: had to `grep -rn "implements GitClient" src/ test/` and verify each copy manually. `agents-versions.it.test.ts`'s double was missed, compiled clean, and returned HTTP 500 at runtime when `container.context.listPaths` invoked the undefined `cloneExists`. Fix: add the methods to every copy.
Why it matters: `server/tsconfig.json`'s `include` is `src/**/*.ts` only — `pnpm typecheck` never type-checks `test/**`, so a test double that no longer satisfies the interface it `implements` passes typecheck and fails only when the code path runs (and only in whichever suite exercises it). When widening any port, grep `implements <Port>` across `test/` and update every copy by hand; the compiler will not save you.

## 2026-07-12 — A run reaches terminal status before its trace row is written (two tables, non-atomic)
Context: L05 T4's it.test fetched `GET /runs/:id/trace` right after `waitForPrRuns` returned a `done` run. Green in isolation, flaky (2→3→4 failures) only under the full concurrent it.test suite; failures were fast (~400ms) `getTrace` `statusCode !== 200`, not timeouts.
What we tried: assumed `status === 'done'` implied the trace document was queryable.
What worked: made the test's `getTrace` poll the endpoint until 200 (5s budget). Root cause: `run-executor.ts` `completeAgentRun` writes `agent_runs.status='done'` (~line 282) BEFORE `saveRunTrace` writes the separate `run_traces` row; `waitForPrRuns` polls only `agent_runs`, so under DB load the trace row lags the terminal run.
Why it matters: `agent_runs` (status) and `run_traces` (doc) are distinct tables written non-atomically — a terminal run is NOT a guarantee its trace is readable. Any test or consumer reading the trace immediately after a run goes terminal must poll/retry. The pre-existing `run-executor-skills.it.test.ts` never hit this because it asserts via `mockLlm.calls`, not the trace endpoint. If this recurs and matters for the UI, the fix is to reorder the executor to `saveRunTrace` before flipping status. `server/src/modules/reviews/run-executor.ts:282`, `server/test/run-executor-context.it.test.ts`.

## 2026-07-12 — Clone-reading tests need a real-dir GitClient double, not `MockGitClient`
Context: L05's context reader globs/stats/reads real files under the clone; tests must exercise real fs behavior (symlink-escape, `../` traversal, discovery).
What we tried: `MockGitClient` — but its `clonePathFor()` returns a synthetic `/mock/clones/...` path that doesn't exist on disk, so it can't exercise any real filesystem behavior.
What worked: a small `RealDirGitClient implements GitClient` over an `fs.mkdtemp` dir, with the app's `DEVDIGEST_CLONE_DIR` set to that tmp root. The latter is mandatory because `ContextService.resolveClone` now asserts the resolved `cloneRoot` is under `config.cloneDir` (a path-traversal defense) — an unanchored tmp dir is (correctly) rejected as "not cloned".
Why it matters: for any feature that reads the on-disk clone, `MockGitClient` is useless; copy the `RealDirGitClient` fixture pattern from `context-reader.it.test.ts` and anchor its root under `config.cloneDir`, or `listPaths`/discovery silently returns "not cloned". `server/src/adapters/mocks.ts`, `server/test/context-reader.it.test.ts`.

## 2026-07-13 — Finding severity is `CRITICAL/WARNING/SUGGESTION`; `BLOCKER_SEVERITIES` carries a dead `blocker` value
Context: designing the Why+Risk Brief `riskLevel` floor ("any blocker/critical finding forces `riskLevel` ≥ high"). The spec author, the design doc, and the implementation plan all independently gated on a severity literal `'blocker'`.
What we tried: exact-string checks like `severity === 'blocker' || severity === 'critical'`.
What worked: reuse `BLOCKER_SEVERITIES = new Set(['blocker','critical'])` matched via `.toLowerCase()` (`server/src/modules/overview/brief/aggregate.ts:41,82`). The real finding severity enum is `z.enum(['CRITICAL','WARNING','SUGGESTION'])` (`server/src/vendor/shared/contracts/findings.ts:11`) — there is NO `'blocker'` severity, and `'critical'` is uppercase. So `'blocker'` in that Set is dead code, and the set only ever matches `'CRITICAL'` (via lowercasing).
Why it matters: an exact-string `severity === 'blocker'` (or lowercase `'critical'`) gate compiles, passes a unit test seeded with fabricated lowercase severities (the DB `severity` column is untyped text), and then silently never fires on real data — a dead guard on a security-relevant path. Any severity-tier gate MUST reuse the case-insensitive `BLOCKER_SEVERITIES` set, and its test MUST use a real `'CRITICAL'` row. Three separate agents made this exact mistake in one feature; only an adversarial plan review caught it.

## 2026-08-25 — `eval_runs` is per-case by construction (`case_id NOT NULL`) and cannot hold a set-level run
Context: designing L06's "run the agent over all cases and compare two runs". The shipped `eval_runs` table carries `recall`/`precision`/`citation_accuracy`/`cost_usd` columns, which reads like a run-level results table.
What we tried: writing one `eval_runs` row per set-run, with the per-case detail folded into `actual_output`.
What worked: it cannot be done — `eval_runs.case_id` is `NOT NULL` and FKs to `eval_cases` (`server/src/db/schema/eval.ts:24-35`, `0000_init.sql:122-127`), so a row is definitionally one case executed once. The set-level concept needs its own relation. Landed as a new `eval_run_batches` table plus a nullable `batch_id` on `eval_runs`; the alternative (nullable-ing `case_id` and overloading the table) was rejected because the run's system-prompt snapshot would then be duplicated across every row of a run and could drift between them.
Why it matters: the metric columns on `eval_runs` invite the assumption that it is the run table, and the shipped `EvalRun` contract (`contracts/knowledge.ts:59-73`, with `traces_passed`/`traces_total`/`per_trace[]`) reinforces it by describing a set-level shape. The two disagree, and the schema is the binding one. Check `case_id`'s nullability before designing anything set-shaped on top of `eval_runs`.

## 2026-08-25 — A finding's diff fragment is reconstructable offline from `pr_files.patch` — no GitHub call, no clone
Context: L06's "turn a finding into an eval case in one click" has to store the diff fragment the finding refers to. The obvious source is the PR diff, which `loadDiff` fetches via `container.git.diff` — a clone-dependent call that fails or blocks on a repo that was never cloned.
What we tried: planning to fetch the PR diff at case-creation time and slice the finding's file out of it.
What worked: `loadDiff` (`server/src/modules/reviews/diff-loader.ts:12-29`) already treats git as best-effort and falls back to `diffFromPrFiles`, which rebuilds a unified diff purely from the persisted `pr_files.patch` column. Restricting that reconstruction to a single path gives the fragment with a plain DB read. On this workspace it covers everything: all 97 findings had a matching `pr_files` row with a non-null patch. The string-assembly half is now `assembleDiffFragment` in `server/src/modules/_shared/diff-fragment.ts`, hoisted there because onion MUST.4 forbids a second module importing `modules/reviews`.
Why it matters: any feature that needs "the diff behind this finding" should read `pr_files.patch` rather than reach for the git adapter — it works with no clone, no network and no credentials, which is the difference between a one-click action and one that fails on a cold workspace. The caveat is that `pr_files.patch` is nullable, so the no-stored-patch branch needs a real error path rather than an empty fragment (an empty fragment parses to zero files and would be scored as a legitimate zero-finding run).

## 2026-08-25 — A single `MockLLMProvider` returns the same fixture for every call, making multi-call batch assertions vacuously green
Context: integration-testing an eval batch that replays N stored diffs through `reviewPullRequest`, one LLM call per case, then scores recall/precision/citation across the whole set.
What we tried: one `MockLLMProvider` instance for the batch, configured with a structured response.
What worked: subclassing it and overriding `completeStructured` to pick a fixture by matching the case's `Eval case · <name>` task line out of the assembled prompt (the `task` section `reviewer-core/src/prompt.ts` places in the user message). Deterministic regardless of the order the DB returns rows in, and it still makes no real model call.
Why it matters: the stock double has no per-call variation, so all N cases come back with identical findings. Every metric still computes, the suite is green, and the assertions are worthless — recall and precision can only be exercised by cases that differ from each other. This is worse than a missing test, because it reads as coverage. Any suite that drives more than one model call in a single run needs a provider double that varies its answer, and the metric expectations must be hand-computed from the fixtures rather than read back from the implementation.

## 2026-08-25 — `modules/eval` duplicates two queries verbatim; a change to either source silently de-tunes the eval harness
Context: onion MUST.4 forbids `modules/eval` importing `modules/agents` or `modules/reviews`, so its repository re-implements `enabledSkillBodiesForAgent` (`server/src/modules/agents/repository.ts:282-290`) and `findingContext` (`server/src/modules/reviews/repository/review.repo.ts:103-117`) as its own Drizzle queries.
What we tried: leaning on the precedent at `server/src/modules/reviews/repository/run.repo.ts:23`, which joins `t.agents` directly. An architecture review accepted the reading as MUST-clean but flagged that the precedent is weaker than it looks — that one is a single-column display join for a name, while these copy whole filtering and ordering bodies line for line.
What worked: leaving the duplication in place with comments naming each source, and recording this entry instead of hoisting to `_shared/` mid-run (which would have required editing `modules/agents`, outside the feature's scope). Multi-tenant safety is NOT the exposure here — that guard lives at the service layer (`server/src/modules/eval/service.ts:167`, `:182`), which was verified rather than assumed.
Why it matters: the real exposure is silent de-tuning. If `enabledSkillBodiesForAgent` ever changes which skills it selects, production reviews get the new set and eval runs keep the old one — the harness quietly stops mirroring the thing it exists to regression-test, and no compiler error or failing test connects the two. When touching either source query, grep `server/src/modules/eval/repository.ts` for its copy. If a third consumer ever appears, hoist all of them into `modules/_shared/` the way `assembleDiffFragment` was hoisted in this same feature.
- 2026-08-25 · Tool & Library Notes · `POST /agents/:id/eval-runs` is a multi-minute synchronous request: cases replay in a sequential loop (provider rate limits preclude parallelism) and the route awaits the whole batch before responding — a 10-case set measured ~6 min wall clock against claude-sonnet-4-6. server/src/modules/eval/service.ts:279, server/src/modules/eval/routes.ts:83. Reason: the batch row is inserted with status 'running' *before* the loop (service.ts:263), so an HTTP client that gives up on its default timeout still gets a completed batch in history a few minutes later — the run is not cancelled with the request, and a retry silently starts a second full batch.
- 2026-08-25 · Codebase Patterns · The eval dashboard's metric tiles blank to `—` for the whole duration of a run, then refill: `last_run` is the newest batch by `ran_at` regardless of status (service.ts:491), and a batch row is inserted with NULL metrics before the first LLM call (service.ts:263). server/src/modules/eval/service.ts:491. Reason: neither hunk shows it alone — the regression is only visible once you know a batch runs for minutes, and it reads as data loss rather than as an in-flight row outranking the last succeeded one.
- 2026-08-26 · What Doesn't Work · `eval_runs.recall` / `.precision` / `.citation_accuracy` are never populated — `insertRun` omits all three from its values list, so 47/47 rows hold NULL regardless of pass/fail. server/src/modules/eval/repository.ts:378. Reason: the columns are pre-staged schema and the batch-level metrics ARE written, so the gap is invisible unless you query the run rows directly; `scoreCase` deliberately returns only pass/matchedFindingId/truePositives/falsePositives, so there is nothing to write without extending the scorer first.
- 2026-08-27 · What Doesn't Work · `computeAlert` compares the two newest batches by recency alone, with no regard for whether they scored the same case set — a 12-case run preceded by a 1-case run yields a banner ("recall dropped by 0.13; precision dropped by 0.27") whose numbers contradict the compare modal on the same screen, because the deltas span different denominators. server/src/modules/eval/helpers.ts:95. Reason: the doc comment states the >= 0.02 threshold but not the set-equality assumption behind it, and the alert looks authoritative sitting above the metric tiles.
- 2026-08-27 · Codebase Patterns · Two different sources of truth for "the PR diff": reviews call `loadDiff`, which prefers a real `git diff base...head` from the local clone and only falls back to `pr_files`, while every other consumer (eval case creation, for one) reads `pr_files` directly. server/src/modules/reviews/diff-loader.ts:19. Reason: the divergence is invisible until something looks a reviewer-cited file up in `pr_files` and does not find it — which reads as a hallucinated citation but is not. Diagnostic: compare `pull_requests.files_count` against `count(*)` in `pr_files`; rows persisted before 2026-08-27 stop at exactly 100 because `pulls.listFiles` was called unpaginated, and a re-sync is needed to backfill them.
