...
- 2026-08-27 · Codebase Patterns · A finding only reaches precision's denominator if it survives the grounding gate: `scoreBatch` divides true positives by `kept` findings, while dropped ones go to `citation_accuracy` instead (kept/(kept+dropped)). reviewer-core/src/eval/score.ts:. Reason: it decides whether a deliberately noisy prompt actually tests precision — instruct the model to cite lines INSIDE the diff and the noise lands in precision (16%→6% observed); let it cite anywhere and grounding silently absorbs the same noise into citation accuracy, leaving precision untouched and the experiment measuring the wrong thing.
